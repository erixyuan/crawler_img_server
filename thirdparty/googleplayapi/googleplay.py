...
import gp_pb2 as googleplay_pb2
import config
import random
...
class GooglePlayAPI(object):
    ...
    def __init__(self, androidId=None, device=None, lang=None, debug=False, proxies=None): # you must use a device-associated androidId value
        self.preFetch = {}
        if androidId == None:
    ...
    def login(self, email=None, password=None, authSubToken=None, timeout=20):
        ...
        if (authSubToken is not None):
            ...
            response = requests.post(self.URL_LOGIN, data=params, headers=headers, 
                verify=False, timeout=timeout, proxies=self.proxies)
            ...
            for d in data:
                ...
                fields = d.split("=")
                if len(fields) != 2:
                    err_str = "bad fields:[%s], status:[%s], response:[%s]" % (
                        d, response.status_code, response.text)
    ...
    def executeRequestApi2(self, path, datapost=None, post_content_type="application/x-www-form-urlencoded; charset=UTF-8", timeout=20):
        if (datapost is None and path in self.preFetch):
            ...
            headers = { "Accept-Language": self.lang,
                                    ...
                                    "User-Agent": "Android-Finsky/3.7.13 (api=3,versionCode=8013013,sdk=16,device=%s,hardware=herring,product=soju)" % (self.device),
    ...
    def download(self, packageName, versionCode, offerType=1, timeout=30):
        ...
        #cookie_value = ""
        #for i in xrange(20):
        #    cookie_value = cookie_value + str(random.randint(0,9))

        #cookies = { 
        #    'MarketDA': cookie_value # python-requests #459 fixes this
        #}  
